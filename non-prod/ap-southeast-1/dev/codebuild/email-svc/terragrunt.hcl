locals {
  ...
  service_repo = local.environment_vars.locals.email_service_repo
  build_branch = local.environment_vars.locals.build_branch
  project_name = local.environment_vars.locals.project_name
}
dependencies {
  paths = ["../../vpc", "../../iam-policy", "../../secret_manager", "../../codecommit/email-svc"]
}

dependency "email-svc" {
  config_path = find_in_parent_folders("codecommit/email-svc")
  mock_outputs = {
...
inputs = {
  ...
  codecommit_be_url      = dependency.email-svc.outputs.codecommit_be_url
