locals {
  ...
  service_repo = local.environment_vars.locals.cognito_service_repo
  build_branch = local.environment_vars.locals.build_branch
  project_name = local.environment_vars.locals.project_name
}
dependencies {
  paths = ["../../vpc", "../../iam-policy", "../../secret_manager", "../../codecommit/cognito-trigger"]
}

dependency "cognito-trigger" {
  config_path = find_in_parent_folders("codecommit/cognito-trigger")
  mock_outputs = {
...
inputs = {
  ...
  codecommit_be_url      = dependency.cognito-trigger.outputs.codecommit_be_url
