locals {
  ...
  service_repo = local.environment_vars.locals.viz_erp_reactjs_repo
  build_branch = local.environment_vars.locals.build_branch
  project_name = local.environment_vars.locals.project_name
}
dependencies {
  paths = ["../../vpc", "../../iam-policy", "../../secret_manager", "../../codecommit/viz-erp-reactjs"]
}

dependency "ts-common" {
  config_path = find_in_parent_folders("codecommit/viz-erp-reactjs")
  mock_outputs = {
...
dependency "iam-policy" {
  ...
  mock_outputs = {
    codebuild_arn_1 = "arn:aws:iam::111111111111:role/codebuild-role"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan", "init"]
}
...
inputs = {
  ...
  codecommit_be_url      = dependency.ts-common.outputs.codecommit_be_url
